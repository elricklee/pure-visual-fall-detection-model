...
def _load_context() -> dict:
    ...
    model_paths = {
        ...
        "pt": ROOT / "artifacts" / "pytorch" / "fall_pose_384_best.pt",
    }
    return {
...
def build_member_b_report(context: dict, output_path: Path) -> None:
    ...
    _add_table(
        ...
        ["类别", "交付物", "状态"],
        [
            ["代码", "scripts/data/create_multi_person_fixture.py", "已完成"],
            ["代码", "tools/reports/summarize_multi_validation.py", "已完成"],
...
def build_project_status_report(context: dict, output_path: Path) -> None:
    ...
    _add_table(
        ...
        [
            ...
            ["scripts/runtime/infer_video.py", "单/多人标注视频推理", "已完成"],
            ["scripts/evaluation/evaluate_videos.py", "批量视频评估；多人指标按帧去重", "已整改"],
            ["scripts/runtime/run_system.py", "事件中心、告警、截图、回放、trace、报告", "已整改"],
            ["scripts/data/create_multi_person_fixture.py", "多人功能测试素材生成", "新增完成"],
            ["tools/reports/summarize_multi_validation.py", "多人运行汇总", "新增完成"],
        ],
        [3000, 4380, 1980],
    )
    ...
    _add_table(
        ...
        [
            ...
            ["fall_pose_384_best.pt", f"{models['pt']['mib']:.3f} MiB", "训练后PyTorch模型", "已完成"],
        ],
        [4200, 1500, 2100, 1560],
    )
